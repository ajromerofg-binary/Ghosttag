...
banner() {
    ...
    echo "   ╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚══════╝   ╚═╝      ╚═╝   ╚═╝  ╚═╝ ╚═════╝ "
    echo ""
    echo "   Source Code + Document + Binary Credential Hunter v2.0"
...
# ── Log helpers ───────────────────────────────────────────────────────────────
log_info()    { $QUIET || echo -e "${GREEN}[+]${NC} $*"; }
log_warn()    { $QUIET || echo -e "${YELLOW}[!]${NC} $*"; }
log_error()   { echo -e "${RED}[-]${NC} $*" >&2; }
log_section() { $QUIET || echo -e "\n${CYAN}${BOLD}[*] $*${NC}"; }
log_doc()     { $QUIET || echo -e "${BLUE}[DOC]${NC} $*"; }
...
validate() {
    ...
    if [[ -n "$OUTPUT_FILE" ]]; then
        ...
        OUTPUT_FILE="ghosttag_$(date +%Y%m%d_%H%M%S).${FORMAT}"
    fi
    ...
    # Avisar de dependencias opcionales ausentes
    local missing=()
    command -v pdftotext &>/dev/null || missing+=("pdftotext (poppler-utils) — PDF text")
    command -v exiftool  &>/dev/null || missing+=("exiftool — metadatos PDF/Word/Excel/imagen")
    command -v antiword  &>/dev/null || missing+=("antiword — Word .doc legacy")
    command -v sqlite3   &>/dev/null || missing+=("sqlite3 — analisis de BBDD SQLite")
    python3 -c "import docx" &>/dev/null 2>&1 || missing+=("python3-docx (pip) — Word .docx")
    python3 -c "import openpyxl" &>/dev/null 2>&1 || missing+=("openpyxl (pip) — Excel .xlsx")

    if [[ ${#missing[@]} -gt 0 ]]; then
        log_warn "Dependencias opcionales ausentes (modulos afectados no se ejecutaran):"
    ...
    $INSECURE && log_warn "SSL check     : DESACTIVADO (-k)"
    [[ -n "$COOKIES" ]]      && log_info "Cookies       : configuradas"
    [[ -n "$EXTRA_HEADER" ]] && log_info "Header extra  : configurado"
}
...
build_curl_flags() {
    ...
    $INSECURE && CURL_FLAGS+=(--insecure)
    [[ -n "$COOKIES" ]]      && CURL_FLAGS+=(--cookie "$COOKIES")
    [[ -n "$EXTRA_HEADER" ]] && CURL_FLAGS+=(--header "$EXTRA_HEADER")
}
    ...
    for pattern_entry in "${PATTERNS[@]}"; do
        ...
        if [[ -n "$matches" ]]; then
            while IFS= read -r match; do
                ...
                matched_text=$(echo "$match" | cut -d: -f2- | sed 's/^[[:space:]]*//' | head -c 200)
                findings+=("${severity}|${category}|${description}|${linenum}|${matched_text}")
                (( hit_count++ )) || true
            done <<< "$matches"
        fi
    done
...
write_finding() {
    ...
    if ! $QUIET; then
        echo -e "  ${color}${BOLD}[$severity]${NC} ${BOLD}$description${NC}"
        echo -e "           ${CYAN}Linea/Campo $linenum${NC} => $matched_text"
        echo ""
    fi
    ...
    if [[ "$FORMAT" == "txt" ]]; then
        ...
        safe_text=$(echo "$matched_text" | sed 's/\\/\\\\/g; s/"/\\"/g')
    ...
    if command -v pdftotext &>/dev/null; then
        ...
        if [[ -s "$text_file" ]]; then
            ...
            if (( count > 0 )); then
                ...
                local idx=0
                while IFS='|' read -r sev cat desc lnum mtxt; do
                    (( idx++ )) || true
                    [[ $idx -eq 1 ]] && continue
                    [[ -z "$sev" ]] && continue
    ...
    if command -v exiftool &>/dev/null; then
        ...
        if [[ -n "$meta" ]]; then
            ...
            declare -A META_FIELDS=(
                ["Author"]="MEDIUM|METADATA|Autor del documento"
            ...
            for field in "${!META_FIELDS[@]}"; do
                local value
                value=$(echo "$meta" | grep -E "^${field}[[:space:]]*:" | cut -d: -f2- | sed 's/^[[:space:]]*//' | head -c 200 || true)
                if [[ -n "$value" && "$value" != "-" ]]; then
                    IFS='|' read -r sev cat desc <<< "${META_FIELDS[$field]}"
                    findings_meta+=("${sev}|${cat}|${desc}: ${value}|meta|${value}")
                    (( meta_hit++ )) || true
                fi
            done
...
PYEOF
    ...
    if [[ -n "$meta_json" ]]; then
        python3 - "$meta_json" "$url" << 'PYEOF2' 2>/dev/null || true
import sys, json
...
for field, (sev, cat, desc) in sev_map.items():
    ...
    if val and val not in ("None", "0", ""):
        print(f"METAHIT|{sev}|{cat}|{desc}|meta|{val}")
PYEOF2
    fi

    # Procesar METAHIT del bloque python anterior
    # (capturado via subshell arriba, pero necesitamos re-ejecutar para capturar stdout)
    local meta_hits
    meta_hits=$(python3 - "$meta_json" 2>/dev/null << 'PYEOF3' || true
import sys, json, os

raw = sys.stdin.read().strip()
# meta_json viene del entorno, leerlo del primer arg si existe
try:
...
PYEOF
    ...
    if [[ -n "$meta_json" ]]; then
        local meta_hits
        meta_hits=$(python3 - "$meta_json" << 'PYEOF2' 2>/dev/null || true
import sys, json
try:
    meta = json.loads(sys.argv[1])
except Exception:
...
content = content.rstrip().rstrip(']') + summary
with open(path, 'w') as f:
    f.write(content)
PYEOF
        local py_exit=$?
        (( py_exit != 0 )) && echo "]}" >> "$OUTPUT_FILE"
    fi
}
...
process_url() {
    ...
    if [[ "$rtype" == "html" || "$rtype" == "sqldump" ]]; then
        ...
        if [[ "$rtype" == "sqldump" ]]; then
            analyze_sqldump "$url" "$content" > /dev/null
        else
            local analysis
            analysis=$(analyze_source "$url" "$content")
            local hit_count
    ...
    # ── Recursos binarios / documentos ───────────────────────────────────────
        ...
        local hits=0
        case "$rtype" in
            pdf)          hits=$(analyze_pdf      "$url" "$tmpfile") ;;
            word)         hits=$(analyze_word_docx "$url" "$tmpfile") ;;
            word_legacy)  hits=$(analyze_word_doc  "$url" "$tmpfile") ;;
            excel|excel_legacy) hits=$(analyze_excel "$url" "$tmpfile") ;;
            image)        hits=$(analyze_image    "$url" "$tmpfile") ;;
            sqlite)       hits=$(analyze_sqlite   "$url" "$tmpfile") ;;
        esac
...
print_summary() {
    echo ""
    echo -e "${CYAN}${BOLD}======================================================${NC}"
    echo -e "${BOLD}  GHOSTTAG v2.0 — Resumen${NC}"
    echo -e "${CYAN}${BOLD}======================================================${NC}"
    echo -e "  URLs/recursos analizados : ${BOLD}$TOTAL_URLS${NC}"
    echo -e "  Documentos/binarios      : ${BOLD}$TOTAL_DOCS${NC}"
    echo -e "  Hallazgos totales        : ${BOLD}${RED}$TOTAL_FINDINGS${NC}"
    echo -e "  Reporte                  : ${BOLD}$OUTPUT_FILE${NC}"
    echo -e "${CYAN}${BOLD}======================================================${NC}"
    echo ""
    if (( TOTAL_FINDINGS > 0 )); then
        echo -e "${RED}${BOLD}  [!] Se encontraron credenciales o datos sensibles.${NC}"
        echo -e "${YELLOW}  Revisa el reporte: $OUTPUT_FILE${NC}"
    else
        echo -e "${GREEN}  [OK] No se detectaron patrones sensibles conocidos.${NC}"
    fi
    echo ""
}
