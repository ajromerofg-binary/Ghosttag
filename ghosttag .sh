...
process_url() {
    ...
    # ── Recursos binarios / documentos ───────────────────────────────────────
        ...
        local hits=0
        local _mod_output
        # Capturar output del módulo; la última línea es siempre el número de hits.
        # Los logs (log_doc, log_warn) se emiten dentro de subshell y van a stdout del módulo;
        # extraemos sólo la última línea que es el entero de hits.
        case "$rtype" in
            pdf)               _mod_output=$(analyze_pdf       "$url" "$tmpfile" 2>/dev/null) ;;
            word)              _mod_output=$(analyze_word_docx  "$url" "$tmpfile" 2>/dev/null) ;;
            word_legacy)       _mod_output=$(analyze_word_doc   "$url" "$tmpfile" 2>/dev/null) ;;
            excel|excel_legacy)_mod_output=$(analyze_excel      "$url" "$tmpfile" 2>/dev/null) ;;
            image)             _mod_output=$(analyze_image      "$url" "$tmpfile" 2>/dev/null) ;;
            sqlite)            _mod_output=$(analyze_sqlite     "$url" "$tmpfile" 2>/dev/null) ;;
        esac
        # La última línea del output es el número de hits
        hits=$(echo "$_mod_output" | tail -1 | grep -oE '^[0-9]+$' || echo "0")
        # Reimprimir los logs que quedaron capturados (todo menos la última línea)
        local _mod_logs
        _mod_logs=$(echo "$_mod_output" | head -n -1)
        [[ -n "$_mod_logs" ]] && echo "$_mod_logs"
